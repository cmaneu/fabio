...
| `.agents/skills/fabio-*/SKILL.md` (12 intent-scoped sub-skills) | `data/skills/*.json` (authored judgment) + `commands.json` (command index) | `subskills_match_generated` | New command/subcommand added, or a `data/skills/*.json` family edited |
...
- **Intent-scoped sub-skills (Layer 2, generated)**: Twelve `fabio-<family>` sub-skills (lakehouse, warehouse-sql, data-engineering, dataflows, data-science, mirroring, rti-kql, bi, ontology, deploy-cicd, admin, migration) at `.agents/skills/fabio-*/SKILL.md` are GENERATED from authored judgment (`src/commands/context/data/skills/<family>.json`) + a command index pulled from `commands.json`. This realizes the division of labor: prose carries judgment (when-to-use, gotchas, safety, routing); the command table is mechanically derived (drift-free). The generator lives in `src/commands/context/skillgen.rs` (a `#[cfg(test)]`-only module — it is a build/test-time tool, not runtime, mirroring `generate_agent_schema`). Regenerate with `cargo test generate_subskills -- --ignored`; the `subskills_match_generated` drift test fails in CI if committed files are stale. All workload command groups except the niche `map` (geospatial) are covered by a family or persona. The root `fabio` skill remains the comprehensive single-file entry point and now routes to sub-skills for progressive disclosure (load only the relevant sub-skill to keep context lean).
