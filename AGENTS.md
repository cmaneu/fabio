...
- **updateDefinition CAN switch between PBIR-Legacy and PBIR formats**: Despite earlier assumptions, format conversion works in both directions. Send PBIR parts (definition/version.json, definition/pages/...) to convert to PBIR; send report.json to convert to PBIR-Legacy. However, adding invalid schema fields (like `prototypeQuery` in PBIR visual) causes silent rejection and format stays unchanged.
