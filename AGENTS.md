...
ghcr.io/iemejia/fabio:0.27.0       # release version
...
- **Elements array for table/column selection**: The `elements` field in `datasource.json` defines which tables and columns the agent can access. Each element has `id` (REQUIRED — use schema/table/column path like `"dbo"`, `"dbo.products"`, `"dbo.products.price"`), `display_name`, `type` (e.g., `lakehouse_tables.table`, `lakehouse_tables.column`), `is_selected`, `description`, `data_type`, and `children` (nested). Setting `is_selected: true` makes them available to the agent. Without `elements`, the agent cannot query tables (returns "technical issue accessing" error). Without valid `id` fields, the API returns an error about null/empty IDs.
...
- **FabricOpenAI chat base URL (internal workload)**: The SDK constructs a different chat URL than the public published URL: `{wl_host}/webapi/capacities/{capacityId}/workloads/ML/AISkill/Automatic/v1/workspaces/{workspaceId}/artifacts/{artifactId}/aiassistant/openai`. This internal URL supports both `sandbox` (draft) and `production` stages via the `x-ms-ai-aiskill-stage` header. fabio uses the public published URL (`https://api.fabric.microsoft.com/v1/workspaces/{ws}/dataagents/{id}/aiassistant/openai`) instead.
- **Tagged thread endpoint (persistent conversations)**: `GET {wl_host}/webapi/capacities/{cap}/workloads/ML/AISkill/Automatic/v1/workspaces/{ws}/artifacts/{id}/aiassistant/threads/fabric?agent=aiskill&stage={stage}&client=dataagentsdk&tag={tag}&scenario={scenario}` — creates or retrieves a thread by tag for multi-turn conversations. Delete via `DELETE` on the same URL. fabio currently creates ephemeral threads (one per query) via the standard OpenAI Assistants API.
- **Assistant scenario types**: The internal chat API supports multiple scenario types via `x-ms-ai-assistant-scenario` header: `aiskill` (default for Data Agents), `dscopilot` (Data Science Copilot), `dwcopilot` (Data Warehouse Copilot), `aiskillv1shim` (legacy compatibility). fabio uses the public endpoint which doesn't expose scenario selection.
- **Stage mapping**: The SDK maps user-friendly names to internal values: `draft`/`sandbox` → `sandbox`, `publishing`/`production` → `production`. fabio exposes this as `--stage sandbox|production` on the `query` command.
