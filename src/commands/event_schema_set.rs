...
pub enum EventSchemaSetCommand {
...
pub async fn execute(
    cli: &Cli,
    client: &FabricClient,
    command: &EventSchemaSetCommand,
) -> Result<()> {
    match command {
        EventSchemaSetCommand::List { workspace } => list(cli, client, workspace).await,
        EventSchemaSetCommand::Show { workspace, id } => show(cli, client, workspace, id).await,
        EventSchemaSetCommand::Create {
            workspace,
            name,
            description,
        } => create(cli, client, workspace, name, description.as_deref()).await,
        EventSchemaSetCommand::Update {
            workspace,
            id,
            name,
            description,
...
async fn list(cli: &Cli, client: &FabricClient, workspace: &str) -> Result<()> {
    let resp = client
        .get_list(
            &format!("/workspaces/{workspace}/eventSchemaSets"),
            "value",
            cli.all,
...
async fn show(cli: &Cli, client: &FabricClient, workspace: &str, id: &str) -> Result<()> {
    let data = client
        .get(&format!("/workspaces/{workspace}/eventSchemaSets/{id}"))
        .await?;
...
async fn create(
    ...
    let data = client
        ...
        .await
        .map_err(|e| enrich_forbidden(e, "event-schema-set create", "Contributor"))?;
    output::render_object(cli, &data, "id");
    Ok(())
}
...
async fn update(
    ...
    if name.is_none() && description.is_none() {
        return Err(FabioError::with_hint(
            ...
            "Example: fabio event-schema-set update --workspace <WS> --id <ID> --name \"New Name\""
                .to_string(),
        )
        .into());
    }
    ...
    if output::dry_run_guard(cli, "event-schema-set update", &body) {
        return Ok(());
    }
    let data = client
        .patch(
        ...
        .await
        .map_err(|e| enrich_forbidden(e, "event-schema-set update", "Contributor"))?;
    output::render_object(cli, &data, "id");
    Ok(())
}
