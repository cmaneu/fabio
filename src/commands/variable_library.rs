...
pub async fn execute(
    ...
    match command {
        ...
        VariableLibraryCommand::Delete { workspace, id } => {
            delete(cli, client, workspace, id).await
        }
        VariableLibraryCommand::GetDefinition { workspace, id, decode } => {
...
async fn delete(cli: &Cli, client: &FabricClient, workspace: &str, id: &str) -> Result<()> {
    ...
    client
        .delete(&format!("/workspaces/{workspace}/variableLibraries/{id}"))
        .await
