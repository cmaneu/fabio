...
pub async fn execute(
    ...
    match command {
        ...
        SnowflakeDatabaseCommand::Delete { workspace, id } => {
            delete(cli, client, workspace, id).await
        }
        SnowflakeDatabaseCommand::GetDefinition { workspace, id, decode } => {
...
async fn delete(cli: &Cli, client: &FabricClient, workspace: &str, id: &str) -> Result<()> {
    ...
    client
        .delete(&format!("/workspaces/{workspace}/snowflakeDatabases/{id}"))
        .await
