...
pub enum AnomalyDetectorCommand {
...
pub async fn execute(
    cli: &Cli,
    client: &FabricClient,
    command: &AnomalyDetectorCommand,
) -> Result<()> {
    match command {
        AnomalyDetectorCommand::List { workspace } => list(cli, client, workspace).await,
        AnomalyDetectorCommand::Show { workspace, id } => show(cli, client, workspace, id).await,
        AnomalyDetectorCommand::Create {
            workspace,
            name,
            description,
...
async fn list(cli: &Cli, client: &FabricClient, workspace: &str) -> Result<()> {
    let resp = client
        .get_list(
            &format!("/workspaces/{workspace}/anomalyDetectors"),
            "value",
            cli.all,
...
async fn show(cli: &Cli, client: &FabricClient, workspace: &str, id: &str) -> Result<()> {
    let data = client
        .get(&format!("/workspaces/{workspace}/anomalyDetectors/{id}"))
        .await?;
...
async fn create(
    ...
    let data = client
        ...
        .await
        .map_err(|e| enrich_forbidden(e, "anomaly-detector create", "Member"))?;
    output::render_object(cli, &data, "id");
    Ok(())
}
...
async fn update(
    ...
    if name.is_none() && description.is_none() {
        return Err(FabioError::with_hint(
            ...
            "Example: fabio anomaly-detector update --workspace <WS> --id <ID> --name \"New Name\""
                .to_string(),
        )
        .into());
    }
    ...
    if output::dry_run_guard(cli, "anomaly-detector update", &body) {
        return Ok(());
    }
    ...
    let data = client
        ...
        .await
        .map_err(|e| enrich_forbidden(e, "anomaly-detector update", "Contributor"))?;
    output::render_object(cli, &data, "id");
    Ok(())
}
...
async fn update_definition(
    ...
    let definition_json = match (file, content) {
        ...
        (None, None) => {
            return Err(FabioError::with_hint(
                ...
                "Example: fabio anomaly-detector update-definition --workspace <WS> --id <ID> --file definition.json".to_string(),
            ).into());
        }
    };
    ...
    if output::dry_run_guard(
        cli,
        "anomaly-detector update-definition",
        &serde_json::json!({
            "workspace": workspace,
            "id": id,
    ...
    let data = client
        .post(
            &format!("/workspaces/{workspace}/anomalyDetectors/{id}/updateDefinition"),
            &body,
            true,
        )
        .await
        .map_err(|e| enrich_forbidden(e, "anomaly-detector update-definition", "Contributor"))?;
