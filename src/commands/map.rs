...
pub async fn execute(cli: &Cli, client: &FabricClient, command: &MapCommand) -> Result<()> {
    match command {
        ...
        MapCommand::Delete { workspace, id } => delete(cli, client, workspace, id).await,
        MapCommand::GetDefinition { workspace, id } => {
...
async fn list(cli: &Cli, client: &FabricClient, workspace: &str) -> Result<()> {
    let resp = client
        .get_list(
            &format!("/workspaces/{workspace}/maps"),
            "value",
            cli.all,
...
async fn show(cli: &Cli, client: &FabricClient, workspace: &str, id: &str) -> Result<()> {
    let data = client
        .get(&format!("/workspaces/{workspace}/maps/{id}"))
        .await?;
...
async fn delete(cli: &Cli, client: &FabricClient, workspace: &str, id: &str) -> Result<()> {
    ...
    client
        .delete(&format!("/workspaces/{workspace}/maps/{id}"))
        .await
        .map_err(|e| enrich_forbidden(e, "map delete", "Contributor"))?;
...
async fn get_definition(cli: &Cli, client: &FabricClient, workspace: &str, id: &str) -> Result<()> {
    let data = client
        .post(
            &format!("/workspaces/{workspace}/maps/{id}/getDefinition"),
            &serde_json::json!({}),
            true,
        )
        .await
        .map_err(|e| enrich_forbidden(e, "map get-definition", "Contributor"))?;
...
async fn update_definition(
    ...
    let script = match (file, content) {
        ...
        (None, None) => {
            return Err(FabioError::with_hint(
                ...
                "Example: fabio map update-definition --workspace <WS> --id <ID> --file definition.json".to_string(),
            )
            .into());
        }
    };
    ...
    let data = client
        .post(
            &format!("/workspaces/{workspace}/maps/{id}/updateDefinition"),
            &body,
            true,
        )
        .await
        .map_err(|e| enrich_forbidden(e, "map update-definition", "Contributor"))?;
