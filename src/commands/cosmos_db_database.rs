...
pub async fn execute(
    ...
    match command {
        CosmosDbDatabaseCommand::List { workspace } => list(cli, client, workspace).await,
        CosmosDbDatabaseCommand::Show { workspace, id } => show(cli, client, workspace, id).await,
        ...
        CosmosDbDatabaseCommand::GetDefinition { workspace, id, decode } => {
...
async fn list(cli: &Cli, client: &FabricClient, workspace: &str) -> Result<()> {
    let resp = client
        .get_list(
            &format!("/workspaces/{workspace}/cosmosDbDatabases"),
            "value",
            cli.all,
...
async fn create(
    ...
    let data = client
        ...
        .await
        .map_err(|e| enrich_forbidden(e, "cosmos-db-database create", "Contributor"))?;
    output::render_object(cli, &data, "id");
    Ok(())
}
...
async fn update_definition(
    ...
    let script = match (file, content) {
        ...
        (None, None) => {
            return Err(FabioError::with_hint(
                ...
                "Example: fabio cosmos-db-database update-definition --workspace <WS> --id <ID> --file definition.json".to_string(),
            )
            .into());
        }
    };
    ...
    if output::dry_run_guard(
        cli,
        "cosmos-db-database update-definition",
    ...
    let data = client
        .post(
            &format!("/workspaces/{workspace}/cosmosDbDatabases/{id}/updateDefinition"),
            &body,
            true,
        )
        .await
        .map_err(|e| enrich_forbidden(e, "cosmos-db-database update-definition", "Contributor"))?;
